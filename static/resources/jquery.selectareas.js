...
(function ($) {
    $.imageArea = function (parent, id) {
        var options = parent.options,
            ...
            updateBtMenu = function (visible) {
                if ($btMenu) {
                    $btMenu.css({
                        display: visible ? "block" : "none",
                        left: area.x + area.width,
                        top: area.y - $btMenu.outerHeight() - 3,
                        "z-index": area.z + 3
                    });
                }
            },
            updateBtNumber = function (visible) {
                if ($btNumber) {
                    $btNumber.css({
                        display: visible ? "block" : "none",
                        left: area.x + area.width,
                        top: area.y - $btNumber.outerHeight() - 1,
                        "z-index": area.z + 5
                    });
                }
            },
            updateMenuSelectBox = function (visible) {
                if ($menuSelectBox) {
                    $menuSelectBox.css({
                        display: visible ? "block" : "none",
                        left: area.x + area.width + 3,
                        top: area.y - $btMenu.outerHeight() - 3,
                        "z-index": area.z + 3
                    });
                }
            ...
            deleteSelection = function (event) {
                ...
                if ($btDelete) {
                    $btDelete.remove();
                    $btMenu.remove();
                    $menuSelectBox.remove();
                }
                parent._remove(id);
                fireEvent("changed");
            },
            menuSelection = function (event) {
                $(".menu-area").click(function () {

                    if ($(".period").css("display") == "none") {
                        console.log("clicked");
                        $(".period").css("display", "inline-block");
                    } else if ($(".period").css("display") == "inline-block") {
                        console.log("clicked2");
                        $(".period").css("display", "none");
                    }
                })
            },
            numberSelection = function (event) {
                var image_name='';
                let image= {
                    id: 0,
                    name: '',
                ...
                $(".number-area").click(function () {
                    //image name'i alalım
                    $.getJSON("../json/instantData.json",function (data){
                        image_name=data[0].imageName; 
                    }).then(function (x){
                        $.getJSON("../json/savedData.json",function (data){
                            data.forEach(function(nesne,index){
                                if(nesne.id==area.id && nesne.name==image_name){
                                    image.id=nesne.id;
                                    ...
                                    image.w=nesne.w;
                                    image.h=nesne.h;
                                    image.font=nesne.font;
                                }  
                            })
                        }).then(function (x){    
                            $.ajax({
                                type: "POST",
                                url: '../back/processCroppedImage.php',
                                data: image,
                            });
                        })
                    })  
        ...
        selectBoxSelection = function (event) {
        ...
        if (options.allowMenu) {
            var bindToMenu = function ($obj) {
                $obj.click(menuSelection)
                    .bind("touchstart", menuSelection)
                    .bind("tap", menuSelection);

                return $obj;
            };
            $btMenu = bindToMenu($("<div class=\"menu-area\" />"))
                .append(bindToMenu($("<div class=\"font-selection\" />")))
                .insertAfter($selection);

        }
        // initialize number button
        if (options.allowNumber) {
            var bindToNumber = function ($obj) {
                $obj.click(numberSelection)
                    .bind("touchstart", numberSelection)
                    .bind("tap", numberSelection);
                return $obj;
            };
            $btNumber = bindToNumber($("<div class=\"number-area\" />"))
                .append(bindToNumber($("<div class=\"select-number-areas\"/>")))
                .insertAfter($selection);
        }

        // initialize menu select box
        if (options.allowSelectBox) {
            var bindToSelectBox = function ($obj) {
                $obj.click(selectBoxSelection)
                    .bind("touchstart", selectBoxSelection)
                    .bind("tap", selectBoxSelection);

                return $obj;
            };

            $menuSelectBox = bindToMenu($("<div class=\"selection-area\" />"))
                .append(bindToMenu($(` 
            <div class="period" >
                <span class="input">
        ...
        return {
            ...
            selectBoxSelection: selectBoxSelection,
            options: options,
            blur: blur,
            focus: focus,
    ...
    $.imageSelectAreas.prototype.init = function (object, customOptions) {
        ...
        if (!this.options.allowEdit) {
            this.options.allowSelect = this.options.allowMove = this.options.allowResize = this.options.allowDelete = this.options.allowMenu = this.options.allowNumber = this.options.allowSelectBox = false;
        }
    ...
    $.imageSelectAreas.prototype.add = function (options) {
        ...
        this._refresh();
        if (!this.options.allowSelect && !this.options.allowMove && !this.options.allowResize && !this.options.allowDelete && !this.options.allowMenu && !this.options.allowNumber && !this.options.allowSelectBox) {
            this.blurAll();
        }
    };
